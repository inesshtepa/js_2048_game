...
function render() {
    ...
    if (game.getStatus() === 'playing' && maxValue > 0) button.classList.add(`button--${maxValue}`);
...
class Game {
    ...
    moveDown() {
        this.moveColumn((col)=>this.mergeRow([
                ...col
            ].reverse()).reverse());
    }
    getEmptyBoard() {
    ...
    move(mergeCallback) {
        ...
        if (!this.hasBoardChanged(oldState, this.state)) return;
        this.addRandomTile();
        this.updateStatus();
    }
    ...
    canMove() {
        for(let row = 0; row < 4; row++)for(let col = 0; col < 4; col++){
            ...
            if (this.state[row][col + 1] === cell) return true;
            if (this.state[row + 1]?.[col] === cell) return true;
        }
        return false;
    }
}
